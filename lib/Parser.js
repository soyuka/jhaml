...
Parser.prototype._transform = function(chunk, encoding, callback) {
  ...
  if(this.indentation === null && this.line === 0) {
...
Parser.prototype.processLine = function(chunk) {
  ...
  for(let i = 0; i <= length; i++) {
    ...
    if(began) {
      ...
      } else if(!element.started() && !~[this.encode_interpolated, this.interpolate_encode, this.code, this.exclamation, this.filters].indexOf(chunk[i])) {
  ...
  if(element.filter) {
    let filter = bufferToString(chunk, element.filterStart, element.filterEnd)
