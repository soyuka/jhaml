...
Javascript.prototype.open = function(element, indent) {
  ...
  if(!element.code && this.started && space) {
    prefix = '\\n'+prefix
  }
  ...
  if(element.code) {
    this.currentEngine.push(prefix + element.open())
  } else if(element.tag.open !== null) {
...
Javascript.prototype.close = function(element, indent, test) {
  ...
  if(element.content === false && !element.code && element.whitespaceremoval.after === false) {
    prefix = '\\n' + spaces(indent)
  }
