...
/**
 * Is the buffer an opening/closing quote?
 * @param {Buffer} chunk 
 * @param {Number} i 
 * @param {Number} previousQuote 
 */
Parser.prototype.isQuote = function(chunk, i, previousQuote) {
  let quote = chunk[i]

  if(!(quote === this.singleQuote || quote === this.doubleQuote))
    return false

  if(previousQuote === null || previousQuote === quote) {
...
Parser.prototype.processLine = function(chunk) {
  ...
  for(let i = 0; i <= length; i++) {
    //toggle quote
    if(this.isQuote(chunk, i, quote)) {
      inQuote = !inQuote 
      quote = !inQuote ? null : chunk[i]
    }
    ...
    if(began) {
      ...
      if(element.started() && chunk[i - 1] !== this.space && chunk[i] === this.attributes.start) {
        attributes.start = i
      ...
      if(!element.started() && (!chunk[i-2] || chunk[i-2] === this.space)) {
        let isElement = ~[this.id, this.classname, this.element].indexOf(chunk[i - 1])
      ...
      //element has not started and this is not an haml reserved character
      } else if(!element.started() && !~[this.encode_interpolated, this.interpolate_encode, this.code, this.exclamation].indexOf(chunk[i])) {
        element.contentOnly = true
        element.closed = true
        content = i - 1
        break
      }
  ...
  /*
   * if(!element.closed && element.started && !element.ended() && !this.haml.handled) {
   *   let isElement = ~[this.id, this.classname, this.element].indexOf(chunk[element.start])
   *   if(!isElement) {
  ...
  if(this.startline === null && (element.started() || (element.contentOnly && element.content)) && !element.code) {
